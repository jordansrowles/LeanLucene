...
/// <summary>
/// Compares block-join query performance: LeanCorpus BlockJoinQuery vs
...
public class BlockJoinBenchmarks
{
    ...
    public int LeanCorpus_IndexBlocks()
    {
        var path = Path.Combine(Path.GetTempPath(), $"leancorpus-bench-block-{Guid.NewGuid():N}");
    ...
    [Benchmark]
    [MethodImpl(MethodImplOptions.NoInlining)]
    public int LeanCorpus_BlockJoinQuery()
    {
    ...
    {
        _leanIndexPath = Path.Combine(Path.GetTempPath(), $"leancorpus-bench-block-s-{Guid.NewGuid():N}");
